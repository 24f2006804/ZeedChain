...
contract EquityNFTFactory is ERC721, Ownable {
    ...
    function getInvestorInvestments(address investor) external view returns (uint256[] memory) {
        return _founderStartups[investor].values();
    }
    ...
    function getRegisteredStartups() external view returns (uint256[] memory) {
        uint256 total = _nextTokenId;
        uint256[] memory registeredIds = new uint256[](total);
        uint256 count = 0;
        
        for (uint256 i = 1; i <= total; i++) {
            if (_exists(i)) {
                registeredIds[count] = i;
                count++;
            }
        }
...
// File contracts/FractionalInvestment.sol
...
contract FractionalInvestment is ERC1155, Ownable, ReentrancyGuard {
    ...
    function getInvestorPortfolio(address investor) external view returns (uint256[] memory) {
        uint256[] memory registeredStartups = equityFactory.getRegisteredStartups();
        uint256[] memory portfolio = new uint256[](registeredStartups.length);
        uint256 count = 0;

        for (uint256 i = 0; i < registeredStartups.length; i++) {
            uint256 startupId = registeredStartups[i];
            if (balanceOf(investor, startupId) > 0) {
                portfolio[count] = startupId;
                count++;
            }
        }
...
// File contracts/StakeholderGovernance.sol
