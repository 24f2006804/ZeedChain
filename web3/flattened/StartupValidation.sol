...
// File contracts/StartupValidation.sol
...
contract StartupValidation is Ownable {
    ...
    function getStartups() external view returns (uint256[] memory) {
        uint256 total = equityFactory.totalStartups();
        uint256[] memory allStartups = new uint256[](total);
        uint256 count = 0;
        
        for (uint256 i = 1; i <= total; i++) {
            try equityFactory.getStartupDetails(i) returns (EquityNFTFactory.Startup memory) {
                allStartups[count] = i;
                count++;
            } catch {
                // Skip if startup doesn't exist
            }
        }
